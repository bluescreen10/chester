...
func main() {
	...
	generatePreamble(buf, bookFile)
	generateRandoms(buf)
	book := readBookFile(bookFile)
	generateBook(buf, book)
...
func generatePreamble(w io.Writer, file string) {
	fmt.Fprintf(w, "// Code generated from %s. DO NOT EDIT.\n\n", file)
	fmt.Fprintf(w, "//go:generate go run internal/cmd/book/main.go\n\n")
...
func generateRandoms(w io.Writer) {
	...
	for color := range 2 {
		fmt.Fprintf(w, "\t\t{\n")
		for piece := range 6 {
			polyglotPiece := (color ^ 0x01) + piece*2
			fmt.Fprintf(w, "\t\t\t{\n")
			for row := 7; row >= 0; row-- {
				for file := range 8 {
					offset := polyglotPiece*64 + row*8 + file
					fmt.Fprintf(w, "\t\t\t\t0x%x,\n", random[offset])
				}
			}
			fmt.Fprintf(w, "\t\t\t},\n")
	...
	// black to move
	fmt.Fprintf(w, "\tWhiteToMove: 0x%x,\n", random[780])
...
func readBookFile(path string) []bookEntry {
	...
	if err != nil {
		log.Fatalf("readBookFile: error opening file %v", err)
	}

	for i := 0; i < len(bin); i += 16 {

		key := binary.BigEndian.Uint64(bin[i : i+8])
