...
func generatePreamble(w io.Writer) {
	...
	fmt.Fprintf(w, "//go:generate go run internal/cmd/const/main.go\n\n")
...
func generateRays(w io.Writer) {
	...
	fmt.Fprintf(w, "var straightRays [Square(64)]Bitboard = [Square(64)]Bitboard{\n")
	for i := range 64 {
		fmt.Fprintf(w, "\t%d,\n", r[0][i]|r[1][i]|r[2][i]|r[3][i])
	}
	fmt.Fprintf(w, "}\n")

	fmt.Fprintf(w, "var diagonalRays [Square(64)]Bitboard = [Square(64)]Bitboard{\n")
	for i := range 64 {
		fmt.Fprintf(w, "\t%d,\n", r[4][i]|r[5][i]|r[6][i]|r[7][i])
	}
	fmt.Fprintf(w, "}\n")
}
...
// FIXME: the order should be A1 B1 ... H8
